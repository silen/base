...
class WxPayApi
{
	private $notify_url; //保护通知地址

	public function __construct()
	{
		$this->notify_url = C('site_url') . 'Wap/Index/wxpayNotifyMultiVendor';
	}
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid(C('PAY_APPID'));//公众账号ID
		$inputObj->SetMch_id(C('PAY_MCHID'));//商户号
	...
	{
		//如果不需要上报数据
		if(C('REPORT_LEVENL') == 0){
			return;
		} 
		//如果仅失败上报
		if(C('REPORT_LEVENL') == 1 &&
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, C('PAY_SSLCERT_PATH'));
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, C('PAY_SSLKEY_PATH'));
		}
		//post提交方式
